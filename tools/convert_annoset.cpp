...
DEFINE_bool(enforce_anno_format, false,
"Enforce AnnotatedDatum type even for classification tasks (ignored for detection).");
...
int main(int argc, char** argv) {
	...
	const bool enforce_anno_format = FLAGS_enforce_anno_format;
	...
	//read the filenames and label
	if (anno_type == "classification") {
		while (infile >> filename >> labelname) {
			lines.push_back(std::make_pair(filename, name_to_label[labelname]));
		}
	...
	for (int line_id = 0; line_id < lines.size(); ++line_id) {
		...
		// Put in db
		string out;
		if ( (anno_type == "classification") && !enforce_anno_format) {
			CHECK(datum->SerializeToString(&out));
		} else if ( (anno_type == "detection") || (enforce_anno_format) ) {
